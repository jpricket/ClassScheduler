...
public class HtmlSchedulePrinter {
    ...
    public void printToFile(final String filename) throws IOException {
        ...
        if (blocks != null && blocks.size() > 0) {
            ...
            for (final Block b : blocks) {
                ...
                sb.append(String.format("            toggleBlock({ locationId: \"%s\", height: %d, id: \"%s\", name: \"%s\", color: \"%s\", opacity:%f,\n" +
                                "                hint: \"%s\"\n            });\n",
                        b.locationId, b.height, b.id, b.caption, b.getColorAsString(), b.opacity, b.hint));
            }
            // End the last function
            sb.append("        }\n");
        }
    ...
    private void addBlock(final CourseSchedule.ClassTime time, final CourseDescriptor course, final Color color,
        ...
        final String hint = String.format("%s (%s)\\n%s %s - %s", course.getTitle(), course.getRefNumber(),
                time.Day.toString(), getDisplayTime(time.Hour, time.Minute, 0),
                getDisplayTime(time.Hour, time.Minute, time.DurationMinutes));
        ...
        final Block b = new Block(blockId, getRowId(timeIndex, dayIndex), time.DurationMinutes,
                course.getName(), color, hint, opacity, classDescription);
        if (scheduled) {
            this.blocks.add(b);
        } else {
